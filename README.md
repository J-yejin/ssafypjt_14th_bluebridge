...
* 커밋 메시지는 **영문 버전 + 한국어 버전**으로 작성한다
* **영문이 원본**, 한국어는 **영문 내용의 번역본**
* 줄 수는 **강제하지 않는다**

  * 제목 1줄
  * 내용은 **0 ~ 최대 3줄까지 허용**
  * 한국어 버전은 영문 내용 전체를 번역

👉 **불필요하게 늘리는 것은 금지**,
👉 **설명할 게 있으면 최대 3줄까지 허용**
...
<제목 (영문, 1줄)>
<영문 내용 (0~3줄)>
<한국어 번역 (영문 내용 전체 번역, 줄 수 자유)>
```

※ 영문 내용이 없으면 한국어 번역도 생략 가능
...
* 반드시 **영문**
* 동사는 **원형(add, implement, fix, update 등)**
* 마침표 사용 ❌
* 너무 포괄적인 표현 금지
...
```text
feat: add password confirmation to signup serializer
```

---

### 4️⃣ 영문 내용(Body) 작성 규칙

#### 목적

* 제목만으로 부족한 **구현 맥락 보완**
* “무엇을 / 어떻게 / 왜” 중 필요한 부분만 작성
...
```text
Validate password and password_confirm fields before user creation
Remove password_confirm field before saving user instance
```
...
```text
회원가입 시 사용자 생성 이전에 비밀번호와 비밀번호 확인 필드를 검증함
사용자 저장 전에 비밀번호 확인 필드를 제거함
```
...
```text
chore: update gitignore and requirements
```
...
### 8️⃣ 다른 GPT에 붙여넣을 때 추천 문구

```text
아래 커밋 메시지 규칙을 반드시 준수해서 커밋 메시지를 작성해줘.
- 제목은 영문, Conventional Commits 형식
- 본문은 최대 3줄 영문
- 마지막에 영문 본문의 한국어 번역 포함
```
